...
export class AdminController {
    ...
    static async createProject(req: CascataRequest, res: any, next: any) {
        ...
        try {
            ...
            const tempClient = new pg.Client({ 
                connectionString: `postgresql://${user}:${pass}@${dbHost}:${dbPort}/${dbName}`
            });
    ...
    static async updateProject(req: CascataRequest, res: any, next: any) {
        try {
            ...
            if (custom_domain !== undefined) { fields.push(`custom_domain = $${idx++}`); values.push(safeDomain); }
            if (log_retention_days !== undefined) { fields.push(`log_retention_days = $${idx++}`); values.push(log_retention_days); }
            if (ssl_certificate_source !== undefined) { fields.push(`ssl_certificate_source = $${idx++}`); values.push(safeSource); }
            ...
            if (metadata && metadata.db_config) {
                 try {
                     await PoolService.reload(updatedProject.db_name, {
                         ...
                         idleTimeoutMillis: metadata.db_config.idle_timeout_seconds ? metadata.db_config.idle_timeout_seconds * 1000 : undefined
                     });
                 } catch(e) { console.error(`[System] Warning: Pool reload failed`, e); }
            }
    ...
    static async deleteProject(req: CascataRequest, res: any, next: any) {
        ...
        try {
            ...
            try { await systemPool.query(`DROP DATABASE IF EXISTS "${project.db_name}"`); } 
            catch { 
                await systemPool.query(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = $1`, [project.db_name]); 
